...
import InputGroupCheckbox from '../components/input/InputGroupCheckbox.vue'
  ...
  <LayoutSingleColumn title="Simultaneous Audio Record Tool">
    <CustomSection>
      ...
      <FormBase @submit.prevent="run">
        ...
        <p v-else>{{ inputDeviceInfo.label }}</p>
        <SelectGroup
    ...
    <CustomSection>
      ...
      <FormBase @submit.prevent="run">
        ...
        <textarea id="input-analysis-description" v-model.trim="form.description" required></textarea>
        <InputGroup
        ...
        />
        <template #actions>
          <ButtonBase type="submit">Analyse on all connected devices</ButtonBase>
        </template>
      </FormBase>
    </CustomSection>
